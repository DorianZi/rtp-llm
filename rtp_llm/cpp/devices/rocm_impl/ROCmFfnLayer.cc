...
MoeDispatchOutput ROCmDevice::epDispatch(const MoeDispatchParams& params) {
    const auto torch_stream = (params.overlapped && init_params_.enable_comm_overlap) ? *torch_comm_stream_ : *torch_default_stream_;
    at::hip::HIPStreamGuardMasqueradingAsCUDA guard(torch_stream);

    DevicePerfWrapper wrapper(this, "epDispatch");
    // std::cout << "use_deepep_moe: " << init_params_.use_deepep_moe << std::endl;
    ...
    if (params.overlapped) {
        // for TBO: wait for reserved microbatch's compute event
        if (params.compute_stream_event) {
            ...
            // for non-TBO overlap: wait for compute stream now
            cudaEvent_t event;
